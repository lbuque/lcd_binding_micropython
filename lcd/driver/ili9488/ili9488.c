...
typedef struct _mp_lcd_ili9488_obj_t {
    mp_obj_base_t base;
    // mp_obj_t bus_obj;
...
} mp_lcd_ili9488_obj_t;


STATIC void set_rotation(mp_lcd_ili9488_obj_t *self, uint8_t rotation)
{
...
STATIC void mp_lcd_ili9488_print(const mp_print_t *print,
                                 mp_obj_t          self_in,
                                 mp_print_kind_t   kind)
{
    (void) kind;
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
    mp_printf(
        print,
...
mp_obj_t mp_lcd_ili9488_make_new(const mp_obj_type_t *type,
                                 size_t               n_args,
                                 size_t               n_kw,
                                 const mp_obj_t      *all_args)
{
    ...
    // create new object
    mp_lcd_ili9488_obj_t *self = m_new_obj(mp_lcd_ili9488_obj_t);
...
STATIC mp_obj_t mp_lcd_ili9488_deinit(mp_obj_t self_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
    ...
    // m_del_obj(mp_lcd_ili9488_obj_t, self);
    return mp_const_none;
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mp_lcd_ili9488_deinit_obj, mp_lcd_ili9488_deinit);


STATIC mp_obj_t mp_lcd_ili9488_reset(mp_obj_t self_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mp_lcd_ili9488_reset_obj, mp_lcd_ili9488_reset);


STATIC mp_obj_t mp_lcd_ili9488_init(mp_obj_t self_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mp_lcd_ili9488_init_obj, mp_lcd_ili9488_init);


STATIC mp_obj_t mp_lcd_ili9488_custom_init(mp_obj_t self_in, mp_obj_t cmd_list_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
    mp_int_t id = 0;
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(mp_lcd_ili9488_custom_init_obj, mp_lcd_ili9488_custom_init);


STATIC mp_obj_t mp_lcd_ili9488_bitmap(size_t n_args, const mp_obj_t *args_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(mp_lcd_ili9488_bitmap_obj, 6, 6, mp_lcd_ili9488_bitmap);


STATIC mp_obj_t mp_lcd_ili9488_mirror(mp_obj_t self_in,
...
STATIC MP_DEFINE_CONST_FUN_OBJ_3(mp_lcd_ili9488_mirror_obj, mp_lcd_ili9488_mirror);


STATIC mp_obj_t mp_lcd_ili9488_swap_xy(mp_obj_t self_in, mp_obj_t swap_axes_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(mp_lcd_ili9488_swap_xy_obj, mp_lcd_ili9488_swap_xy);


STATIC mp_obj_t mp_lcd_ili9488_set_gap(mp_obj_t self_in,
                                       mp_obj_t x_gap_in,
                                       mp_obj_t y_gap_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_3(mp_lcd_ili9488_set_gap_obj, mp_lcd_ili9488_set_gap);


STATIC mp_obj_t mp_lcd_ili9488_invert_color(mp_obj_t self_in, mp_obj_t invert_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(mp_lcd_ili9488_invert_color_obj, mp_lcd_ili9488_invert_color);


STATIC mp_obj_t mp_lcd_ili9488_disp_off(mp_obj_t self_in, mp_obj_t off_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(mp_lcd_ili9488_disp_off_obj, mp_lcd_ili9488_disp_off);


STATIC mp_obj_t mp_lcd_ili9488_backlight_on(mp_obj_t self_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mp_lcd_ili9488_backlight_on_obj, mp_lcd_ili9488_backlight_on);


STATIC mp_obj_t mp_lcd_ili9488_backlight_off(mp_obj_t self_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mp_lcd_ili9488_backlight_off_obj, mp_lcd_ili9488_backlight_off);


STATIC mp_obj_t mp_lcd_ili9488_width(mp_obj_t self_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
    return mp_obj_new_int(self->width);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mp_lcd_ili9488_width_obj, mp_lcd_ili9488_width);


STATIC mp_obj_t mp_lcd_ili9488_height(mp_obj_t self_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
    return mp_obj_new_int(self->height);
}
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mp_lcd_ili9488_height_obj, mp_lcd_ili9488_height);


STATIC mp_obj_t mp_lcd_ili9488_rotation(mp_obj_t self_in, mp_obj_t rotation_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(mp_lcd_ili9488_rotation_obj, mp_lcd_ili9488_rotation);


STATIC mp_obj_t mp_lcd_ili9488_vscroll_area(size_t n_args, const mp_obj_t *args_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(mp_lcd_ili9488_vscroll_area_obj, 4, 4, mp_lcd_ili9488_vscroll_area);


STATIC mp_obj_t mp_lcd_ili9488_vscroll_start(size_t n_args, const mp_obj_t *args_in)
{
    mp_lcd_ili9488_obj_t *self = MP_OBJ_TO_PTR(args_in[0]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(mp_lcd_ili9488_vscroll_start_obj, 2, 3, mp_lcd_ili9488_vscroll_start);


STATIC const mp_rom_map_elem_t mp_lcd_ili9488_locals_dict_table[] = {
    { MP_ROM_QSTR(MP_QSTR_deinit),        MP_ROM_PTR(&mp_lcd_ili9488_deinit_obj)        },
    { MP_ROM_QSTR(MP_QSTR_reset),         MP_ROM_PTR(&mp_lcd_ili9488_reset_obj)         },
    { MP_ROM_QSTR(MP_QSTR_init),          MP_ROM_PTR(&mp_lcd_ili9488_init_obj)          },
    { MP_ROM_QSTR(MP_QSTR_custom_init),   MP_ROM_PTR(&mp_lcd_ili9488_custom_init_obj)   },
    { MP_ROM_QSTR(MP_QSTR_bitmap),        MP_ROM_PTR(&mp_lcd_ili9488_bitmap_obj)        },
    { MP_ROM_QSTR(MP_QSTR_mirror),        MP_ROM_PTR(&mp_lcd_ili9488_mirror_obj)        },
    { MP_ROM_QSTR(MP_QSTR_swap_xy),       MP_ROM_PTR(&mp_lcd_ili9488_swap_xy_obj)       },
    { MP_ROM_QSTR(MP_QSTR_set_gap),       MP_ROM_PTR(&mp_lcd_ili9488_set_gap_obj)       },
    { MP_ROM_QSTR(MP_QSTR_invert_color),  MP_ROM_PTR(&mp_lcd_ili9488_invert_color_obj)  },
    { MP_ROM_QSTR(MP_QSTR_disp_off),      MP_ROM_PTR(&mp_lcd_ili9488_disp_off_obj)      },
    { MP_ROM_QSTR(MP_QSTR_backlight_on),  MP_ROM_PTR(&mp_lcd_ili9488_backlight_on_obj)  },
    { MP_ROM_QSTR(MP_QSTR_backlight_off), MP_ROM_PTR(&mp_lcd_ili9488_backlight_off_obj) },
    { MP_ROM_QSTR(MP_QSTR_height),        MP_ROM_PTR(&mp_lcd_ili9488_height_obj)        },
    { MP_ROM_QSTR(MP_QSTR_width),         MP_ROM_PTR(&mp_lcd_ili9488_width_obj)         },
    { MP_ROM_QSTR(MP_QSTR_rotation),      MP_ROM_PTR(&mp_lcd_ili9488_rotation_obj)      },
    {MP_ROM_QSTR(MP_QSTR_vscroll_area),   MP_ROM_PTR(&mp_lcd_ili9488_vscroll_area_obj)  },
    {MP_ROM_QSTR(MP_QSTR_vscroll_start),  MP_ROM_PTR(&mp_lcd_ili9488_vscroll_start_obj) },
    { MP_ROM_QSTR(MP_QSTR___del__),       MP_ROM_PTR(&mp_lcd_ili9488_deinit_obj)        },
